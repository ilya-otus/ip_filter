...
BOOST_AUTO_TEST_CASE(ip_item_comparison_test) {
    ip_filter::IpItem item1("1.1.1.2\t1\t2"s);
    ip_filter::IpItem item2("1.1.1.1\t2\t1"s);
...
BOOST_AUTO_TEST_CASE(ip_item_equality_test) {
    ip_filter::IpItem item1("1.1.1.1\t1\t2"s);
    ip_filter::IpItem item2("1.1.1.1\t2\t1"s);
    BOOST_CHECK(!(item1 > item2));
    BOOST_CHECK(!(item1 < item2));
}
...
BOOST_AUTO_TEST_CASE(ip_item_parsing_test) {
    ip_filter::IpItem item("1.2.3.4\t1\t2"s);
    std::vector<std::string> rawItem({"1"s,"2"s,"3"s,"4"s});
